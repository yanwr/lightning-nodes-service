...
use crate::{
    errors::AppError,
    nodes::features::list::{NodeResponse, list_nodes},
    state::AppState,
};
...
mod test {
    ...
    use crate::{infras::test_context::TestContext, nodes::model::Node, routes::build_routes};
    ...
    #[tokio::test]
    async fn should_return_200_when_list_nodes_with_formatted_values(ctx: &mut TestContext) {
        let nodes = vec![Node {
        ...
        Node::replace(&ctx.app_state.postgres_pool, &nodes)
            .await
            .unwrap();

        let app = build_routes(ctx.app_state.clone().into());
        let response = app.oneshot(build_request(None)).await.unwrap();
    ...
    #[test_context(TestContext)]
    #[tokio::test]
    async fn should_return_200_when_list_nodes_with_empty_database(ctx: &mut TestContext) {
        let app = build_routes(ctx.app_state.clone().into());
        let response = app.oneshot(build_request(None)).await.unwrap();
    ...
    #[test_context(TestContext)]
    #[tokio::test]
    async fn should_return_404_when_route_does_not_exist(ctx: &mut TestContext) {
        let app = build_routes(ctx.app_state.clone().into());

        let response = app.oneshot(build_request(Some("/node"))).await.unwrap();
        assert_eq!(response.status(), StatusCode::NOT_FOUND);
    }

    #[test_context(TestContext)]
    #[tokio::test]
    async fn should_return_500_when_database_is_unavailable(ctx: &mut TestContext) {
